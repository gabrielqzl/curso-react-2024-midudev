...
const persistanceLocalStorageMiddleware: Middleware<{ users: UserWithId[] }> =
	(store) => (next) => (action) => {
		next(action);
...
export const store = configureStore({
	...
	// middleware: (getDefatultMiddleware) =>
	// 	getDefatultMiddleware().concat(persistanceLocalStorageMiddleware),
});
